...
# Modified by @rachpt and @zaxtyson, see:
# https://github.com/zaxtyson/LanZouCloud-CMD/issues/59
...
def load_with_keys(domain_name="", keys: list = None) -> (dict, str):
    """Try to load cookies from `the first available browser`, return
    the required key-value pairs and browser name
    """
    cookie = {}
    browser = ""
    for cookie_fn in [edge, firefox]:
        try:
            ...
            if all(map(cookie.get, keys)):
                return cookie, cookie_fn.__name__
        except BrowserCookieError:
            pass
